...
/*   Updated: 2025/04/14 14:56:55 by cschnath         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	word_state2(t_lexer *l)
{
	...
	if (l->input[l->idx] == DOUBLE_QUOTE)
	{
		token.type = TOKEN_WORD_DQUOTED;
		l->idx += ft_char_qadjusted(&l->input[l->idx + 1], DOUBLE_QUOTE) + 1;
	}
	else if (l->input[l->idx] == SINGLE_QUOTE)
	{
		token.type = TOKEN_WORD_SQUOTED;
		l->idx += ft_char_qadjusted(&l->input[l->idx + 1], SINGLE_QUOTE) + 1;
	}
	else
	{
		token.type = TOKEN_WORD_UNQUOTED;
		while (ft_is_unquoted_char(l->input[l->idx]) || ft_is_escaped(l->input,
				l->idx))
			l->idx++;
		l->idx--;
	}
